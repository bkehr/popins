		...
		std::vector<std::pair<CharString, Dna5String> > & contigs)
{
	...
	while (locIt != locEnd)
	{
		while (contigIt->first < (*locIt).loc.contig)
			++contigIt;

		SEQAN_ASSERT_EQ(contigIt->first, (*locIt).loc.contig);
...
contigEndsAlign(LocationInfo & a, LocationInfo & b, std::vector<std::pair<CharString, Dna5String> > & contigs)
{
	...
	{
		unsigned prefixLengthA = _min(200, length(itA->second));
		...
		if ((b.loc.chrOri && !b.loc.contigOri) || (!b.loc.chrOri && b.loc.contigOri))
		{
			...
			unsigned prefixLengthB = _min(200, length(itB->second));
		...
		else
		{
			...
			unsigned suffixBeginPosB = length(itB->second) - _min(200, length(itB->second));
	...
	{
		unsigned suffixBeginPosA = length(itA->second) - _min(200, length(itA->second));
...
alignsToRef(LocationInfo & loc,
		...
		FaiIndex & fai)
{
	unsigned dist = 50;			// TODO: Make this a program parameter.
	unsigned preSufLen = 200;	// TODO: Make this a program parameter.
	unsigned extension = 1000;	// TODO: Make this a program parameter.
	...
	std::cout << "contig length: " << length(contigIt->second) << std::endl;
	std::cout << contigIt->second << std::endl;
	...
	if (loc.loc.chrOri)
	{
		ref = loadInterval(fai, loc.loc.chr, loc.loc.chrStart, loc.loc.chrEnd + extension);
		...
		if (loc.loc.contigOri)
		{
			...
			while (align(contigGaps, refGaps, contigSuffix, ref))
			{
				...
				loc.refPos = loc.loc.chrStart + endPosition(refGaps) - 1;
				...
				contigGaps = Gaps<TRcInfix>();
				refGaps = Gaps<Dna5String>();
			}
		}
		else
		{
			...
			if (align(contigGaps, refGaps, contigPrefix, ref))
			{
	...
	{
		ref = loadInterval(fai, loc.loc.chr, loc.loc.chrStart - extension, loc.loc.chrEnd);
		...
		if (loc.loc.contigOri)
		{
			...
			Gaps<TInfix> contigGaps;
			if (align(contigGaps, refGaps, contigSuffix, ref))
			{
				loc.insPos = suffixBeginPos + beginPosition(contigGaps);
				loc.refPos = loc.loc.chrStart + beginPosition(refGaps) - extension;
		...
		FaiIndex & fai)
{
	if (loc.otherEnd == false && loc.insPos != -1)
	{
		...
		if (loc.loc.chrOri)
		{
			rc.loc.chrStart = loc.loc.chrEnd - 100;
			rc.loc.chrEnd = loc.loc.chrEnd + 900;
		}
		else
		{
			rc.loc.chrStart = loc.loc.chrStart - 900;
			rc.loc.chrEnd = loc.loc.chrStart + 100;
		}

		// Process the reverse complemented location.
		if (alignsToRef(rc, contigs, fai))
		{
			if (rc.insPos != -1)
		...
		FaiIndex & fai)
{
	...
	while (it != itEnd)
	{
		...
		else if (alignsToRef(*it, contigs, fai))
		{
		...
		FaiIndex & fai)
{
	...
	while (it != itEnd)
	{
		...
		processOtherEnd(vcfStream, splitAlignLists, (*it)[0], contigs, fai);
		...
		FaiIndex & fai)
{
	...
	findRefAlignedGroups(refAlignedGroups, unaligned, locations, contigs, fai);
	clear(locations);

	// Handle the ref-aligned groups.
	processRefAlignedGroups(vcfStream, groupStream, refAlignedGroups, splitAlignLists, contigs, fai);
	...
	processUnalignedGroups(vcfStream, groupStream, unalignedGroups, splitAlignLists, contigs, fai);
}
		...
		PlacingOptions & options)
{
	...
	setContigLengths(locations, contigs);
	...
	while (it != itEnd)
	{
		if ((*it).loc.chrOri)
		{
			...
			{
				processOverlappingLocs(vcfStream, outGroups, splitAlignLists, fwd, contigs, fai);
				clear(fwd);
			}
			appendValue(fwd, *it);
		...
		else
		{
			...
			{
				processOverlappingLocs(vcfStream, outGroups, splitAlignLists, rev, contigs, fai);
				clear(rev);
			}
			appendValue(rev, *it);
